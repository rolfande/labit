# Lab-IT Tracker — Decap CMS edition
...
This is one of two approaches in this repo — see the [root README](../README.md) for how it
compares to the [github-native/](../github-native/) approach.

- **Data**: one JSON file per lab in [content/labs/](content/labs/), committed to this git repo.
- **Editing**: [Decap CMS](https://decapcms.org/) at `/decap-cms/admin`, which commits changes straight to GitHub.
...
- **Hosting**: GitHub Pages (this repo, `master` branch, root — this folder is served at `/decap-cms/`).
...
Settings → Pages → deploy from `master` / `/ (root)`.

### 2. Create a GitHub OAuth App
GitHub Settings → Developer settings → OAuth Apps → New OAuth App:
- Homepage URL: `https://<you>.github.io/<repo>/decap-cms/`
...
```
cd decap-cms/cloudflare-worker
npx wrangler deploy
npx wrangler secret put GITHUB_CLIENT_ID
npx wrangler secret put GITHUB_CLIENT_SECRET
```
Anyone in the GitHub org/repo with **write access** can now log into `/decap-cms/admin` and
authorize via GitHub — no separate invite list to manage, access is just normal GitHub
...
Repo → Settings → Collaborators → add anyone who should be able to create/edit labs via
`/decap-cms/admin`.

## Day to day use

- Go to `/decap-cms/admin`, log in with GitHub, create/edit a lab and its tasks, hit publish.
  Decap commits the JSON straight to `master`.
- Go to `/decap-cms/` to see the list of all registered labs and overall task progress.
- Go to `/decap-cms/lab.html?id=<lab-id>` to see a single lab's metadata and task checklist.

...
- **Register a lab**: Issues → New issue → **"Register a lab"** template
  ([.github/ISSUE_TEMPLATE/lab.yml](../.github/ISSUE_TEMPLATE/lab.yml)).
- **Tasks**: a markdown checklist in the issue body, one line per task, `@mention` per assignee.
  GitHub shows a live progress bar and lets anyone check off their own task.
...
Settings → Labels → add `status/planned`, `status/in-progress`, `status/completed`,
`status/on-hold` (plus the `lab` label the template already applies). When creating or
...
`Owner`, `Due date` if you want richer filtering/sorting than labels alone. Add a workflow
to auto-add new issues with the `lab` label to the board.
...
- **Register a lab**: Issues → New issue → "Register a lab" template, fill it in, submit.
- **Update tasks**: open the lab's issue, check off completed tasks, add comments for notes.
- **Close a lab out**: apply `status/completed` label and close the issue once all tasks are done.
